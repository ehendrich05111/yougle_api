...
class Test_Backend(unittest.TestCase):
    ...
    def testSingleIndexLow(self):
        result = self.session.post(self.base_url + "deleteSingle/", {"idx": "-1"})
        self.assertEqual(result.status_code, 400)
        self.assertEqual(result.json()["message"], "Index out of range")

    def testSingleIndexHigh(self):
        result = self.session.post(self.base_url + "deleteSingle/", {"idx": "101"})
        self.assertEqual(result.status_code, 400)
        self.assertEqual(result.json()["message"], "Index out of range")
    ...
    def testSingleSuccess(self):
        for i in range(5):
            self.collection.update_one(
                {"email": self.TEST_USER_EMAIL}, {"$push": {"history": "new entry" + str(i)}}
            )
        result = self.session.post(self.base_url + "deleteSingle/", {"idx": "2"})
        self.assertEqual(result.status_code, 200)
        user = self.collection.find_one({"email": self.TEST_USER_EMAIL})
